...
#import "YTBannerLoopView.h"
#import "YTHideSelfWhenTextNilLabel.h"
#import "YTPageControl.h"
#import "UIView+Layout.h"


static const NSTimeInterval YTB_default_Timer_Scroll_Interval = 5.;


@interface YTBannerLoopView () <UIScrollViewDelegate>
...
/////////////////// 底下带遮罩的文字描述 没有内容自动隐藏//////////////////////
@property (nonatomic, strong) YTHideSelfWhenTextNilLabel       * bottomLeftTitleL;
@property (nonatomic, strong) YTHideSelfWhenTextNilLabel       * bottomCenterTitleL;
@property (nonatomic, strong) YTHideSelfWhenTextNilLabel       * bottomRightTitleL;

/////////////////// pageControl ///////////////////////////
@property (nonatomic, strong) YTPageControl *pageContrl;

@end



@implementation YTBannerLoopView{
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        _currentShowIndex = 0;
        _scrollTimerInterval = YTB_default_Timer_Scroll_Interval;
...
- (void)setImageAndTitleWithModel{
    ...
    YTBannerDataModel *leftModel;
    YTBannerDataModel *centerModel = self.bannerModelList[idx];
    YTBannerDataModel *rightModel;
...
- (void)tapImage_action{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(YT_bannerLoopViewClicked:data:)]) {
        [self.delegate YT_bannerLoopViewClicked:_currentShowIndex%_maxImageIndex data:nil];
    }
...
- (void)setBannerModelList:(NSArray<YTBannerDataModel *> *)bannerModelList{
...
- (void)setPageControlOriginType:(YTBannerPageControlOriginType)pageControlOriginType{
    _pageControlOriginType = pageControlOriginType;
    
    if (pageControlOriginType == YTBannerPageControlOriginType_rightBottom) {
        
        self.pageContrl.frame = CGRectMake(self.width / 2., self.height - 33, self.width / 2. - 20, 33);
        
    }else if (pageControlOriginType == YTBannerPageControlOriginType_bottom){
        
        self.pageContrl.frame = CGRectMake(0, self.height - 33, self.width, 33);
        
    }else if (pageControlOriginType == YTBannerPageControlOriginType_top){
...
- (YTPageControl *)pageContrl{
    if (!_pageContrl) {
        _pageContrl = [[YTPageControl alloc]initWithFrame:CGRectMake(self.width / 2., self.height - 33, self.width / 2. - 20, 33)];
...
- (YTHideSelfWhenTextNilLabel *)bottomLeftTitleL{
    if (!_bottomLeftTitleL) {
        _bottomLeftTitleL = [[YTHideSelfWhenTextNilLabel alloc]initWithFrame:CGRectMake(20, self.height - 33., self.width - 20, 33)
...
- (YTHideSelfWhenTextNilLabel *)bottomCenterTitleL{
    if (!_bottomCenterTitleL) {
        _bottomCenterTitleL = [[YTHideSelfWhenTextNilLabel alloc]initWithFrame:CGRectMake(self.width + 20, self.height - 33., self.width - 20, 33)
                                                               withSuperView:self.mainScrollView
                                                                   backColor:nil
                                                                   textColor:nil
                                                                        font:nil];
...
- (YTHideSelfWhenTextNilLabel *)bottomRightTitleL{
    if (!_bottomRightTitleL) {
        _bottomRightTitleL = [[YTHideSelfWhenTextNilLabel alloc]initWithFrame:CGRectMake(self.width * 2. + 20, self.height - 33., self.width - 20, 33)
                                                               withSuperView:self.mainScrollView
                                                                   backColor:nil
                                                                   textColor:nil
                                                                        font:nil];
...
@implementation YTBannerDataModel
